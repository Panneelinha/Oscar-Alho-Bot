...
class SupabaseSyncClient:
    ...
    async def movie_rating_summary(self, movie_id: str) -> tuple[float | None, int]:
        target = quote(movie_id, safe="")
        data = await self._request(
            "GET",
            (
                f"/movie_rating_totals?movie_id=eq.{target}"
                "&select=site_rating_sum,site_rating_count,bot_rating_sum,bot_rating_count"
                "&limit=1"
            ),
        )
        if not isinstance(data, list) or not data:
            return None, 0
        row = data[0]
        total = int(row.get("site_rating_count") or 0) + int(row.get("bot_rating_count") or 0)
        if total <= 0:
            return None, 0
        rating_sum = int(row.get("site_rating_sum") or 0) + int(row.get("bot_rating_sum") or 0)
        return round(rating_sum / total, 1), total
